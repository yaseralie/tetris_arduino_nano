...
unsigned long dropInterval = 600;   // awal lambat
const unsigned long minDropInterval = 100; // batas minimum speed
...
void clearLines() {
  for (int y = PLAY_H - 1; y >= 0; y--) {
    ...
    if (full) {
      ...
      // === Tambahan: tingkatkan kecepatan setiap 10 poin ===
      if (score % 10 == 0 && dropInterval > minDropInterval) {
        dropInterval -= 50; // makin cepat
      }
...
void scrollText(const char *txt, int speed = 10) {
  int len = strlen(txt);
  uint8_t cBuf[8];
...
void resetGame() {
  ...
  score = 0;
  gameOver = false;
  dropInterval = 600; // reset speed setiap game baru

  if (first_time==false){
    scrollText("TETRIS", 30); // scrolling sebelum mulai
  }
  first_time = false;
  spawnPiece();
  lastDrop = millis();
...
void setup() {
  ...
  scrollText("TETRIS", 20); // pertama kali power on
  first_time = true;
  resetGame();
}
